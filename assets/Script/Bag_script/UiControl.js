...
cc.Class({
    ...
    //跳转（英雄面板）
    onClickItem: function(target, data) {
    	var index = data.index
        this.board.active = true
        //初始化详情信息
        var info = HeroInfoData[index]
        this.nameLabel1.string = info.name  //英雄外号
        this.nameLabel2.string = info.name2 //英雄本名
    ...
    onLoad: function() {
    	...
    	for(var i = 0;i < HeroInfoData.length;i ++)
    	{
            ...
            item.getComponent("ListCell").upDateList(this.content.getComponent("SpriteFrameMangager")["iconSpriteFrame" + HeroInfoData[i].id],HeroInfoData[i])
            
            //给预制体的按钮，动态添加点击事件
            var eventHandler = new cc.Component.EventHandler();
            eventHandler.target = this; 
            eventHandler.component = "UiControl";
            eventHandler.handler = "onClickItem";
            eventHandler.customEventData = {"index":i};
